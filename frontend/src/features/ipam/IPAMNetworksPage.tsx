...
export default function IPAMNetworksPage() {
  ...
  const [form, setForm] = useState({ name: '', network: '', gateway: '', description: '' });
  ...
  const handleCreate = () => {
    createNet.mutate(form, {
      onSuccess: () => { toast.success(t('ipam.network_created')); setDialogOpen(false); setForm({ name: '', network: '', gateway: '', description: '' }); },
            ...
            <Link key={net.id} to={`/ipam/network/${net.id}`}>
              <Card className="hover:border-blue-500/50 transition-colors h-full">
                <CardContent className="p-4 space-y-3">
                  ...
                  {net.server_name && <p className="text-xs text-muted-foreground">{net.server_name}</p>}
                </CardContent>
              </Card>
            </Link>
          ))}
